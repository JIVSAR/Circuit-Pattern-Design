...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (current_mode == 1) {
        // Mode 1: 3바퀴 Fw, 3바퀴 Bw 무한 반복 사이클
        uint32_t elapsed = now - mode_start_time;
        // 총 사이클 6바퀴 시간
        uint32_t cycle_time = one_rotation_ms * 3 * 2; 
        uint32_t current_cycle_time = elapsed % cycle_time;

        if (current_cycle_time < (one_rotation_ms * 3)) {
            motor_fw = 1; // 앞쪽 절반(3바퀴)동안은 정방향
        } else {
            motor_bw = 1; // 뒤쪽 절반(3바퀴)동안은 역방향
        }
