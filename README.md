...
```c
    ...
    // 모든 버튼 입력 감지 및 모드 즉각 전환 (폴링 기반 논블로킹 제어)
...
### 5-5. 최종 고찰
* **폴링 기반 논블로킹 제어:** HAL_Delay()가 유발하는 블로킹 병목을 해소하고, `HAL_GetTick()`을 중심으로 메인 루프가 막힘없이 무한대로 회전(Sweeping)하며 모든 입출력을 동시에 감시/관리해내는 임베디드 상급 알고리즘의 좋은 예시가 되었다.
